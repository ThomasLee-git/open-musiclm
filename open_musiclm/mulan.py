...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        return F.layer_norm(
            x, x.shape[-1:], default(self.learned_gamma, self.gamma), self.beta
        )
...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, mask=None):
        ...
        # post norm
        # out = self.norm(x + self.to_out(out))
        out = self.to_out(out)
        return out
...
class SoftmaxContrastiveLearning(nn.Module):
    ...
    def forward(self, sims):
        ...
        contrastive_loss = -log(numerator) + 0.5 * (
            log(denominator_i) + log(denominator_j)
        )
...
class AudioSpectrogramTransformer(nn.Module):
    ...
    def forward(self, x, force_no_patch_dropout=False, return_all_layers=False):
        ...
        rounded_height, rounded_width = map(
            lambda args: round_down_nearest_multiple(*args),
        ...
        if (
            self.training
            and self.patch_dropout_prob > 0.0
            ...
            patch_indices_keep = (
                torch.randn(batch, n, device=device)
                .topk(num_patches_keep, dim=-1)
                .indices
            )
        ...
        x, all_layers = self.transformer(
            x, rel_pos_bias=rel_pos_bias, return_all_layers=True
        )
...
class TextTransformer(nn.Module):
    ...
    def forward(
        ...
        if exists(raw_texts):
            x = tokenizer.tokenize(raw_texts, truncate_text=True).to(
                self.device
            )  # TIP: avoid excess context length
        ...
        assert (
            n <= self.max_seq_len
        ), f"text sequence length {n} must be less than {self.max_seq_len}"
...
class MultiLayerContrastiveLoss(nn.Module):
    ...
    def forward(self, *, audio_layers, text_layers):
        ...
        audio_latents = (
            einsum("l b d, l d e -> l b e", audio_embeds, self.audio_latent_weight)
            + self.audio_latent_bias
        )
        ...
        text_latents = (
            einsum("l b d, l d e -> l b e", text_embeds, self.text_latent_weight)
            + self.text_latent_bias
        )
...
class MuLaN(nn.Module):
    ...
    def __init__(
        ...
        if hierarchical_contrastive_loss:
            ...
            self.register_buffer(
                "text_layers_indices",
                interspersed_indices(num_layers, text_transformer.depth),
            )
            self.register_buffer(
                "audio_layers_indices",
                interspersed_indices(num_layers, audio_transformer.depth),
            )
    ...
    def get_text_latents(
        self, texts=None, raw_texts: Optional[List[str]] = None, return_all_layers=False
    ):
    ...
    def forward(
        ...
        assert (
            cosine_sim.shape[0] == cosine_sim.shape[1]
        ), "batch sizes for audio and text are not equal"
...
@beartype_jit
class MuLanQuantized(nn.Module):
    def __init__(
        self,
        *,
        mulan: MuLaN,
        ...
        self.sample_rate = 24000  # use 24k sampling rate
    ...
    def forward(
        ...
        *,
        ...
        """
        Wrapper for mulan module that takes in audio or text and returns the quantized embedding from the respective tower
        """

        assert exists(audio_input) ^ exists(
            text_input
        ), "either audio or text must be provided, but not both"
        if exists(audio_input):
            assert all(
                wave.dim() == 1 for wave in audio_input
            ), f"audio_input must be a list of 1D tensors, but got {audio_input[0].shape}"

        with torch.no_grad():
            self.mulan.eval()
            if exists(audio_input):
                embedding = self.mulan.get_audio_latents(audio_input)
            else:
                embedding = self.mulan.get_text_latents(raw_texts=text_input)
...
def create_mulan_quantized(
    ...
    **kwargs,
    ...
    pkg = torch.load(str(checkpoint_path), map_location="cpu")
    mulan.load_state_dict(pkg["model"])

    mulan_quantized = MuLanQuantized(mulan=mulan, learn_rvq=learn_rvq, **kwargs)

    if exists(rvq_checkpoint_path):
        rvq = torch.load(rvq_checkpoint_path, map_location=device)
        mulan_quantized.rq.load_state_dict(rvq)

    return mulan_quantized
